...
const RegistrationPage = () => {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex items-center justify-center p-6 transition-colors duration-300">
      ...
      >
        <h2 className="text-2xl font-bold mb-6 text-center">Join StartupForge</h2>
        <GoogleLoginButton />
        ...
        <form className="space-y-4" onSubmit={handleRegister}>
          <div className="space-y-2">
            ...
            <Input name="fullName" className="bg-background border-input h-12" placeholder="John Doe" />
          </div>

          <div className="space-y-2">
            <Label>Email</Label>
            <Input name="email" type="email" className="bg-background border-input h-12" placeholder="name@example.com" />
          </div>
          ...
          <div className="space-y-2">
            ...
            <label
              htmlFor="file-upload"
              className={`flex items-center gap-2 cursor-pointer border border-dashed p-3 rounded-xl w-full text-sm transition-all ${
                imageUrl ? "border-green-500 text-green-600" : "border-input text-muted-foreground hover:bg-accent"
              }`}
            >
              {isLoading ? <Loader2 className="animate-spin" size={18} /> : imageUrl ? <CheckCircle2 size={18} /> : <Upload size={18} />}
              {isLoading ? "Uploading..." : imageUrl ? "Image Uploaded" : "Upload Image"}
            </label>
          </div>

          <div className="space-y-2">
            <Label>Password</Label>
            <Input name="password" type="password" className="bg-background border-input h-12" placeholder="••••••••" />
          </div>
              ...
              {["Founder", "Collaborator"].map((r) => (
                <button
                  ...
                  className={`p-3 rounded-xl border transition-all ${
                    role === r ? "border-primary bg-primary/10 text-primary" : "border-input hover:bg-accent"
                  }`}
                >
                  {r}
                </button>
          ...
          <Button type="submit" className="w-full bg-primary text-primary-foreground font-bold h-12 mt-2 hover:opacity-90">
            Create Account
          </Button>
        </form>
      </motion.div>
